...
export class ProjectService {
  ...
  createProjectFromDeal(dealData: any): Observable<Project> {
    ...
    const projectData = {
      projectName: dealData.title || dealData.dealTitle || 'Untitled Project',
      projectCode: `PRJ-${Date.now()}`,
      clientName: dealData.company || dealData.companyName || 'N/A',
      contactPerson: dealData.contactPerson || 'N/A',
      contactPhone: dealData.phone || 'N/A',
      contactEmail: dealData.email || 'noemail@placeholder.com',
      siteAddress: dealData.address || 'Address not provided',
      elevatorType: dealData.elevatorType || dealData.dealDetails || 'Home Lift',
      numberOfFloors: dealData.floors || dealData.NumberOFloors || 0,
      quantity: dealData.quantity || 1,
      projectValue: Number(dealData.dealAmount || dealData.amount || 0),
      startDate: new Date().toISOString().split('T')[0],
      expectedCompletionDate: this.calculateExpectedCompletion(dealData.expectedCloseDate || dealData.expectedClosingDate || new Date().toISOString()),
